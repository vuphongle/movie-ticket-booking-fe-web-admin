...
export const additionalServiceApi = createApi({
    ...
    endpoints: (builder) => ({
        ...
        getAdditionalServicePrice: builder.query<number, string>({
            query: (additionalServiceId) => `../public/additional-services/${additionalServiceId}/price`,
        }),
        getAdditionalServiceItems: builder.query<AdditionalServiceItem[], string>({
            query: (additionalServiceId) => `../public/additional-services/${additionalServiceId}/items`,
            providesTags: (_result, _error, additionalServiceId) => [
                { type: "AdditionalService", id: `items-${additionalServiceId}` },
            ],
        }),
        createAdditionalService: builder.mutation<AdditionalService, CreateAdditionalServiceRequest>({
        ...
        updateAdditionalService: builder.mutation<AdditionalService, { additionalServiceId: string } & UpdateAdditionalServiceRequest>({
